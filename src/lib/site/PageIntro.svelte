  ...
  <div class:pb-14={!subNavigation} class:gap-y-16={!subNavigation} class:pt-20={!backLink} class=" mx-auto px-2 sm:px-6 flex flex-col pt-20 {isCompact ? 'max-w-4xl' : 'max-w-7xl'}">
    ...
    {#if subNavigation}
      <SideScrollIndicator widthOfContent={widthContent} distanceLeft={0} distanceRight={0}>
        <div class="border-t border-contour-weakest py-5 mt-20 flex gap-8 items-ceter">
          {#if subNavigationLabel}
            <p class="uppercase text-xs tracking-widest font-bold mb-4 mt-4 text-text-weaker whitespace-nowrap">{subNavigationLabel}</p>
          {/if}

          <nav class="flex gap-10" bind:clientWidth={widthContent}>
            {#each subNavigation as { href, separator, abstract, label, isActive }}
              <a {href} class="max-w-60 min-w-40 hover:bg-surface-weakest px-3 py-2.5 rounded-sm" class:bg-surface-weakest={isActive}
