<script>
  ...
  $: options_element = compact(options_list.map(({ options, key }) => {
  	...
  	if (!selected) {
  		console.warn(`Default option was not found for »${key}«. Check default value and list of options.`)
  	...
  	if (!find(options, { value: selected })) {
  		console.warn(`Could not find value »${selected}« for »${key}«. Check list of options. Will hide the option.`);
  		return false;
  	}
  	return {
  		options,
  ...
  function handleChange({ detail }) {
  	CURRENT_INDICATOR_OPTIONS_SELECTION.update(old => ({ ...old, ...detail }));
  }
</script>
