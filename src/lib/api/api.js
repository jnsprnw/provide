...
export function handle(
	...
	const geo = params.geo || take(CURRENT_GEOGRAPHY_UID);
	...
	if (!geo || !indicator || scenarios.length === 0) {
		return undefined;
	}
	
	switch (endpoint) {
		case END_IMPACT_TIME:
			addr = scenarios.map(scenario => [geo, scenario, indicator]);
			param = scenarios.map(scenario => ({ geo, indicator, scenarios: [scenario] })) // We need this for the load function
